...
- [x] **生产模式 SMTP 邮件接入**（email_service.py，无 SMTP 时明确报错）
- [x] **后端 pytest 测试套件**（61 用例，覆盖率 ~70%，含工作流冒烟/规则引擎/API 全链路）
- [x] **前端主路径 E2E**（verify-main-flow.mjs：登录→真实 AI 规划→历史→徒步记录）
- [x] **用户流程自测 E2E**（verify-user-flow.mjs：注册/改密/搜索/对比/GPX/背包/记录/媒体上传 11 环节）
- [x] **路线知识库 151 条**（2026-09-01 补充 16 条，含完整分段，审计脚本全量校验）
- [x] **背包方案管理**（backpacks API + 页面：4 套预设一键创建 + 装备挂载）
- [x] **一键质量脚本**（quality-check.ps1 / npm run check）

## 下一步（2026-09-01 接手后更新）

1. **真实用户测试**（P3-1：规划→装备→背包→历史→记录→传照片，demo 账号已就绪）
2. **社区分享**（P3-2 已完成 ✅：免登录分享链接） / **离线地图**（P3-3 已完成 ✅：瓦片+API 缓存）
3. **独库公路南段分段补充**（铁力买提达坂→库车，见 audit 报告）
4. 仓库已与 GitHub 同步（CI 已上线：push 自动跑 pytest + 前端 check，bcrypt==4.0.1 已固定修复 CI 兼容问题）

> 任务清单与进度以 `docs/ROADMAP.md` 为准；路线数据质量见 `docs/route-audit-report.md`。
